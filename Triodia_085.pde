...
void green() {
  ...
  for (int y = 0; y < height; y++ ) {
    for (int x = 0; x < width; x++ ) {
      ...
      if (previousPixelMagenta == false && currentPixelMagenta == true) {
        // we have hit a perimeter
        insidePerimiter = true;
       // println("Inside Perimiter true ! = " + insidePerimiter + " Loc = " + loc);
      }

      if (insidePerimiter == true && previousPixelMagenta == false && currentPixelMagenta == false  && nextPixelMagenta == false) {
